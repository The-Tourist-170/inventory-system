...
cleanup() {
  ...
  kill $INVENTORY_PID $ORDER_PID $PRODUCT_PID $GATEWAY_PID 2>/dev/null || true
  wait $INVENTORY_PID $ORDER_PID $PRODUCT_PID $GATEWAY_PID 2>/dev/null || true
  echo -e "${GREEN}All services stopped.${NC}"
  echo -e "DBs still running. To stop them: ${CYAN}docker compose -f docker-compose.dev.yml down${NC}"
  exit 0
}
